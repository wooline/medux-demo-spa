...
export class ModelHandlers extends BaseModelHandlers<State, RootState> {
  ...
  public async getItemDetail(itemId: string) {
    ...
    await this.dispatch(actions.comments.searchList({articleType: 'photo', articleId: itemDetail.id}));
  }

  // 同时监听初始化INIT和路由变化的action
  // 参数 null 表示不需要监控loading状态，searchList时会监控loading
  @effect(null)
  protected async [`${ModuleNames.photos}/${ActionTypes.M_INIT},${ActionTypes.F_ROUTE_COMPLETE}`]() {
